...
import { ArrowRight, Bell, Clock3, MapPin, RadioTower } from "lucide-react";
...
export async function LiveCampaignOverview({ campaignId, email, timeZone, userId }: { campaignId: string; email?: string | null; timeZone: string; userId: string }) {
  ...
  return <div className="space-y-5">
    <section className="rounded-[28px] border border-white/[0.06] bg-[#181818] p-6 shadow-[rgba(0,0,0,0.45)_0px_8px_24px] lg:p-8">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 max-w-3xl">
          ...
          <p className="mt-3 max-w-[70ch] text-[14px] leading-6 text-[#b8b8b8]">{overview.campaign.description || "No campaign description added."}</p>
          <div className="mt-5 flex flex-wrap gap-2"><InfoPill icon={Clock3} label={overview.campaign.lastCheckedAt ? `Last checked ${formatDateTimeInTimeZone(overview.campaign.lastCheckedAt, timeZone)}` : "Awaiting first completed run"} /><InfoPill icon={RadioTower} label={`${overview.campaign.sources.length} sources`} />{overview.campaign.regions.length ? <InfoPill icon={MapPin} label={overview.campaign.regions.join(", ")} /> : null}</div>
        </div>
    ...
    <section className="rounded-[24px] border border-white/[0.06] bg-[#181818] p-5 lg:p-6"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#777]">Sources monitored</p><div className="mt-4 flex flex-wrap gap-2">{overview.campaign.sources.map((source) => <span className="rounded-full bg-[#111111] px-3 py-2 text-[11px] font-semibold text-[#c7c7c7]" key={source}>r/{source}</span>)}</div></section>
  </div>;
}
